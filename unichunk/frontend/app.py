...
def render_pdf_page(pdf_path, page_no):
    try:
        import fitz  # PyMuPDF
                            ...
                            for ref_idx, (doc, meta) in enumerate(st.session_state['last_refs']):
                                ...
                                if st.session_state.get('open_ref') == (len(st.session_state['chat_history']), ref_idx):
                                    ...
                                    if page_no is not None and pdf_file:
                                        ...
                                        if pdf_path:
                                            try:
                                                img_data = render_pdf_page(pdf_path, page_no)
                                                if img_data:
                                                    st.image(img_data, caption=ref_label, use_column_width=True)
                                                else:
                                                    st.warning(f"Could not render page {page_no} from PDF.")
                                            except Exception as e:
